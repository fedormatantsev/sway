...
**Reason**: There are no wire types to name, and there are no components to scan: a node kind declares its own inlets, so sockets come from the kind rather than from what the node happens to carry. Superseded by `Inlet identity is a field path`.

**Migration**: A socket previously keyed by a wire type path is keyed by the inlet's field path within the node kind.

### Requirement: Connect and disconnect name a type path
**Reason**: Superseded by `Connect and disconnect name a field path`, which additionally states the self-connection and single-connection-replacement rules that a relationship type used to provide silently.

**Migration**: Connect and disconnect commands name two nodes and two field paths instead of one wire type path.

### Requirement: Edges carry the wire type path
**Reason**: Superseded by `Edges carry two field paths and an ordering key`. An edge now names both ends explicitly and carries the ordering key that variadic inlets need.

**Migration**: A painted edge carries the source and destination paths and its key rather than a single type path.
