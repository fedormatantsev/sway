...
- [ ] 1.1 Add `plane_mesh.rs` with a `PlaneMesh { size, horizontal, vertical }`
...
- [ ] 1.2 Unit-test `PlaneMesh`: independent subdivision counts change vertex density on
      the intended axis only, and `(0, 0)` yields four vertices.
- [ ] 1.3 Remove `MeshMaterial3d<StandardMaterial>` from `MeshAsset`'s `#[require]` and
      update the doc comment that justifies it (design D5).
- [ ] 1.4 Give `MaterialFrom` a relationship hook inserting `MeshMaterial3d<StandardMaterial>`
      on connect and removing it on disconnect, so the field copy still has a target.
- [ ] 1.5 Unit-test the hook: connecting supplies the component, disconnecting removes it,
      and a mesh connected to two material kinds in turn carries exactly one at a time.
- [ ] 1.6 Register `PlaneMesh` as authorable and update
      `the_plugin_registers_every_authorable_component`.
- [ ] 1.7 `cargo test -p sway-nodes`

## 2. sway-runtime — the sprite material

- [ ] 2.1 Add `sway-nodes` to `crates/sway-runtime/Cargo.toml` (design D6; mirrors
      `sway-midi`). Confirm the workspace still builds before writing anything against it.
- [ ] 2.2 Add `frame_sequence.rs` with a `FrameSequence { folder, color_space }`
      component, `#[require(FrameSequenceOut, EditorPos)]`, and a
      `FrameSequenceOut { texture: Handle<Image>, layers: u32 }` outlet.
- [ ] 2.3 Write the pure ordering + validation helpers: sort loaded frame paths
...
- [ ] 2.4 Unit-test the ordering helper against a deliberately shuffled input,
...
- [ ] 2.5 `FrameSequence` load system: `AssetServer::load_folder` with
...
- [ ] 2.6 Add `sprite_material.rs` with the `SpriteMaterial` component
...
- [ ] 2.7 Write the pure `layer_index(frame, layers) -> u32` function: `floor` then
      `clamp` into `[0, layers)` — a safeguard, never a wrap (design D4).
- [ ] 2.8 Unit-test `layer_index` against the spec's scenarios: `3.7`→3, `37.5`→29,
      `-1.0`→0 on a 30-layer sequence; and that `layers` of 0 or 1 does not produce an
      out-of-range index.
- [ ] 2.9 Add `sprite_material.wgsl`: a standard Bevy mesh material (model matrix,
...
- [ ] 2.10 Add the shader to `PREPROCESSOR_SHADERS` in `shader_validation.rs`, as
      `sprite_layer.wgsl` already is (it imports Bevy's own view bindings).
- [ ] 2.11 Implement `Material::specialize` flipping `depth_stencil.depth_write_enabled`
...
- [ ] 2.12 Material sync system: build/update the material asset in place on
...
- [ ] 2.13 Insert an explicit `Aabb` inflated by `depth_range` on entities carrying the
      material, so displaced geometry is not culled — not `NoFrustumCulling`.
- [ ] 2.14 Add `SpriteMaterialFrom` (with the insert/remove hook from 1.4's pattern),
...
- [ ] 2.15 `cargo test -p sway-runtime`

## 3. sway-app — content and demo

- [ ] 3.1 Author or generate two frame folders under `crates/sway-app/assets/`: a colour
...
- [ ] 3.2 Extend `demo.sway.ron` with a sprite layer: two `FrameSequence` nodes wired
...
- [ ] 3.3 Place the sprite layer so it interpenetrates an existing demo cube, and add a
      second layer overlapping the first, so both interpenetration cases in the spec are
      reachable by eye.
- [ ] 3.4 Register the runtime's sprite plugin in `build_app`.
