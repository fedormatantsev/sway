...
- [ ] 1.1 Create `crates/sway-viewport-input` (deps: `bevy_math` only) and add it to the workspace members and `[workspace.dependencies]`.
- [ ] 1.2 Move `ViewportInput`, `ViewportButton`, `ViewportKey`, `ViewportModifiers` and `normalize_viewport_pos` into it, with their tests.
- [ ] 1.3 Leave `ViewportInputRx` behind for now (it moves in group 5) but re-home it on the new types.
- [ ] 1.4 Have `sway-graph` re-export the new crate's items temporarily so downstream crates still compile; add `sway-viewport-input` to `sway-editor`, `sway-runtime` and `sway-app`.
- [ ] 1.5 `cargo test -p sway-viewport-input` and `cargo check --workspace`.

## 2. `sway-graph`: node metadata replaces canvas position

- [ ] 2.1 Replace `Node::pos`/`set_pos` with `metadata: HashMap<String, Box<dyn PartialReflect>>` plus `metadata()`/`metadata_mut()`; update `Node::new`/`Node::of` signatures.
- [ ] 2.2 Confirm an annotation write does not dirty the node, and that an annotation reads back as the type it was written with; add both tests.
- [ ] 2.3 `sway-document`: replace `NodeDoc::pos` with the annotation map, serialized through the untyped `ReflectSerializer`/`ReflectDeserializer` against the registry. Sort keys on save so an unchanged document saves byte-identically. Remove every mention of an editor or a canvas position from the crate's types, doc comments and module docs — after this task `sway-document` has no notion that an editor exists.
- [ ] 2.4 `sway-document`: an annotation whose type is not registered is reported through `LoadDiagnostics` and skipped, leaving the rest of the node loaded.
- [ ] 2.5 `sway-document`: bump `FORMAT_VERSION` to 4 and confirm the existing whole-file version check refuses a version-3 file naming both versions.
- [ ] 2.6 `sway-document`: rename the `v3` module directory to `v4` and update `sway-app`'s `sway_document::v3::` imports.
- [ ] 2.7 `sway-document`: tests for annotation round-trip preserving type, an unrecognised key surviving, an unregistered type being reported and skipped, an entry with no annotations, no key having a field of its own, a double save being byte-identical, and a version-3 file being refused by version.
- [ ] 2.8 `sway-editor`: write canvas placement as a `Vec2` annotation under `"pos"`, directly rather than as a deferred edit. Confirm `Vec2` is registered on every path that loads or saves a document, including bare-`TypeRegistry` tests.
- [ ] 2.9 `cargo test -p sway-graph -p sway-document -p sway-editor`.

## 3. `sway-graph`: the mutation API replaces the command enum

- [ ] 3.1 Add `Graph::create(&TypeRegistry, type_path) -> Option<NodeId>`, carrying over the registered-node-kind check and `ReflectDefault` construction from the old `Create` arm.
- [ ] 3.2 Add `Graph::set_field(NodeId, &str, &dyn PartialReflect) -> FieldWrite` with `FieldWrite { Written, Unchanged, Rejected }`: resolve the path within `inlets`, `reflect_equal`, `try_apply`, dirty only on a real write.
- [ ] 3.3 Delete `GraphCommand`, `CommandOutcome`, `apply_graph_command`, `apply_graph_commands` and `GraphRx`; rewrite `command.rs`'s 23 test sites as direct method tests, each keeping its assertion.
- [ ] 3.4 Delete `FieldValue`, `boxed_for`, `int_as` and `write_field` from `sway-graph`.
- [ ] 3.5 Remove `Graph::selection`/`set_selection` and their tests.
- [ ] 3.6 `GraphPlugin` reduces to inserting the `Graph` resource and scheduling `tick_graph` in `GraphTickSet`.
- [ ] 3.7 Drop `crossbeam-channel` from `sway-graph`'s manifest.
- [ ] 3.8 `cargo test -p sway-graph` — the engine is green before any consumer is touched.

## 4. Consumers move onto the mutation API

- [ ] 4.1 `sway-editor`: add an `EditorEdit` enum (create, delete, set field, connect, disconnect, set slot) and a `GraphEditPlugin` owning the receiver and the `PreUpdate` applier that maps each variant onto a `Graph` method.
- [ ] 4.2 `sway-editor`: add the selection resource; move the drop-on-delete behaviour (a deleted node clears the selection) there, driven off `Graph::removed()`.
- [ ] 4.3 `sway-editor`: convert the 37 `GraphCommand::` sites (canvas 19, inspector 12, scene_tree 3, palette 2, reflect_ui 1) to `EditorEdit`.
- [ ] 4.4 `sway-editor`: move the coercion logic into the inspector — control value → field's declared type, with saturating integer narrowing preserved — and add tests for out-of-range clamping and for a value arriving already typed.
- [ ] 4.5 `sway-runtime`'s gizmo and picker: drop the command construction entirely and call `graph.set_field(..)` / the other methods on the `ResMut<Graph>` they already hold (5 sites).
- [ ] 4.6 `sway-app`: add `GraphEditPlugin` in editor builds where it inserted `GraphRx`.
- [ ] 4.7 `cargo test -p sway-editor -p sway-runtime` and `cargo check --workspace`.
...
- [ ] 5.7 Move `ViewportInputRx` from `sway-graph` to where it is drained (group 6), and delete `sway-graph`'s temporary `sway-viewport-input` re-export.
